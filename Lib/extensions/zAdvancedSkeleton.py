...
#Hi my name is droogie, I really hate GUI development.
#(\/)(*;;;;*)(\/) whoop! whoop! whoop! whoop!

class AdvancedSkeleton(Extension):
	INFO = {
			'Name'		:	'Advanced Skeleton',
			'Usage'		:	'This is an Advanced Example Skeleton.\nIt has some examples of different features included in this framework.\nEverything integrated into the GUI is through the actual extensions code.',
			'Author'	:	'http://wiki.python.org/jython/SwingSampler'
	}
	def finishedLaunching(self):
		self.getCallBacks().registerMenuItem("Extension: _Advanced Example Menu Item_", TestHandler(self))
		SwingSampler(self)	
...
class SwingSampler(object):
		def __init__(self, parent):
				...
				self.parent.addTabPanel("Some Basic Widgets", firstTab)
				...
				self.parent.addTabPanel("HTML Fanciness", secondTab)
				...
				try:
					self.htmlPane = swing.JEditorPane("http://portswigger.net/burp/extender/", editable=0, hyperlinkUpdate=self.followHyperlink, preferredSize=(400, 400))
					secondTab.add(swing.JScrollPane(self.htmlPane), "Center")
				except:
					self.parent.printLogTab("You need to be connected to the internet...")					
				...
				self.parent.addTabPanel("Other Widgets", thirdTab)
...
class TestHandler(IMenuItemHandler):
	...
	def menuItemClicked(self, menuItemCaption, messageInfo):
		self.parent.printLogTab("Thanks for clicking on the Example Menu Item!\n")
